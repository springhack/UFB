...
这个工程是一个不接打印机总线的独立缓冲器固件，只保留霍尔缓冲、电机控制、必要校准和本地存储。
...
- `standalone_buffer`
- `standalone_buffer_high_force`

编译命令：

```bash
env PLATFORMIO_CORE_DIR=$PWD/.platformio pio run -e standalone_buffer
```

高阻力版本：

```bash
env PLATFORMIO_CORE_DIR=$PWD/.platformio pio run -e standalone_buffer_high_force
```

编译产物：

```text
.pio/build/standalone_buffer/firmware.bin
```

高阻力版本产物：

```text
.pio/build/standalone_buffer_high_force/firmware.bin
```

## 编译模式说明

- `standalone_buffer`
  默认独立缓冲器版本，基于 `standard(A1)` 风格参数。

- `standalone_buffer_high_force`
  高阻力版本，适合更长、更弯、摩擦更大的 PTFE 路径。
  这个模式会启用接近原始 `high_force_load(P1S)` 的送料参数，并把独立缓冲器的补料/手动进料 PWM 一并提高。
...
```bash
./wchisp flash .pio/build/standalone_buffer/firmware.bin
```
